...
export default function FinalComprehensiveReport() {
  return (
    <Paper
      ...
      date="April 2026"
      abstract="We present a systematic study of 39 algorithms for detecting genetic relatedness directly from raw FASTQ sequencing reads without reference genome alignment. Evaluated across three datasets of increasing difficulty — synthetic family (controlled), GIAB Ashkenazi trio (cross-batch), and CEPH 1463 pedigree (same-population, 6 members, 3 generations) — we identify fundamental limits of k-mer-based similarity and demonstrate that population-aware rare k-mer analysis overcomes them. Our key contribution is Rare-Run P90, which combines rare k-mer filtering (present in fewer than half of sampled individuals) with 90th-percentile run-length scoring, achieving +583% separation on same-population data where 34 of 39 other algorithms fail. This improves 1.7x over our previous best (haplotype continuity at +345%). The symbiogenesis framing — treating genomic regions as co-inherited variant ecosystems — produces the strongest algorithms when focused on rare informative modules. We provide a complete open-source framework with 39 pluggable algorithms, multi-dataset caching, and automatic report generation."
    >
